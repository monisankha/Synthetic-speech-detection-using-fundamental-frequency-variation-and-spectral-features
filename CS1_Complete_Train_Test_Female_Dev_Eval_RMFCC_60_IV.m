...
for i=1:20
    ...
    for j=2:size(temp,2)
        ...
        stats = [N; F];
        IV_Train(:,count)= extract_ivector(stats, ubm, T_female);
    end
...
for i=1:size(ListOfTrialFiles,1)
    ...
    [N,F] = compute_bw_stats(data', ubm);
    stats = [N; F];
    IV_Test= extract_ivector(stats, ubm, T_female);
...
save Score_ASVspoof_Joint_ASV_CM_Female_Dev_mfcc_nvad_rasta_60_GMMUBM_512_T_300.mat Score;
%--------------------------------------------------------------------------
%% eer computation (dev)
clc;
clear;
...
load Score_ASVspoof_Joint_ASV_CM_Female_Dev_mfcc_nvad_rasta_60_GMMUBM_512_T_300.mat
...
fileID = fopen('development_spoofing_female_ivec.txt','w');
...
fileID = fopen('development_spoofing_joint_cm_ASV_female_ivec.txt','w');
...
save Score_ASVspoof_Joint_ASV_CM_Female_Eval_mfcc_nvad_rasta_60_GMMUBM_512_T_300.mat Score;

%% eer computation (eval)
clc;
clear;
fid = fopen('ASV_female_evaluation.ndx');
ListOfTrialFiles  = textscan(fid,'%s','delimiter',sprintf('\n'));
...
load Score_ASVspoof_Joint_ASV_CM_Female_Eval_mfcc_nvad_rasta_60_GMMUBM_512_T_300.mat
...
fileID = fopen('evaluation_spoofing_female_known_attacks_ivec.txt','w');
fprintf(fileID,'%6s %6s %6s %6s %6s %6s \r\n','eer_baseline','eer_s1','eer_s2','eer_s3','eer_s4','eer_s5');
fprintf(fileID,'%6.4f %6.4f %6.4f %6.4f %6.4f %6.4f\r\n',(eer_baseline*100),(eer_s1*100),(eer_s2*100),(eer_s3*100),(eer_s4*100),(eer_s5*100));
fclose(fileID);

fileID = fopen('evaluation_spoofing_female_unknown_attacks_ivec.txt','w');
...
fileID = fopen('evaluation_spoofing_joint_cm_ASV_female_known_attacks_ivec.txt','w');
fprintf(fileID,'%6s %6s %6s %6s %6s \r\n','eer_s1_cm','eer_s2_cm','eer_s3_cm','eer_s4_cm','eer_s5_cm');
fprintf(fileID,'%6.4f %6.4f %6.4f %6.4f %6.4f\r\n',(eer_s1_cm*100),(eer_s2_cm*100),(eer_s3_cm*100),(eer_s4_cm*100),(eer_s5_cm*100));
fclose(fileID);

fileID = fopen('evaluation_spoofing_joint_cm_ASV_female_unknown_attacks_ivec.txt','w');
